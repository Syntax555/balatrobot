...
class IntentManager:
    ...
    def evaluate(self, gs: dict[str, Any] | Any, deck_cards: list[dict] | None) -> IntentEvaluation:
        playable = [card for card in (deck_cards or []) if _is_playing_card(card)]
        if len(playable) < 2:
        ...
        baseline_deck = _baseline_deck(len(playable))
        ...
        for intent in BuildIntent:
            rng = random.Random(_stable_seed(f"{seed_base}|{intent.value}"))
            raw_values[intent] = _simulate_deck_value(playable, jokers, intent, self._trials, rng)
            baseline_rng = random.Random(_stable_seed(f"{seed_base}|baseline|{intent.value}"))
        ...
        confidence = _confidence_from_gap(scores.get(best, 0.0) - scores.get(second, 0.0))
        return IntentEvaluation(
            intent=best,
